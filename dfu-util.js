...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', event => {
        ...
        let calcModel = document.querySelector("#calc_model");
        let versionS1 = document.querySelector("#version_s1");
        let versionS2 = document.querySelector("#version_s2");
        let patchS1 = document.querySelector("#patch_s1");
        let patchS2 = document.querySelector("#patch_s2");
        ...
        async function connect(device) {
            ...
            if (device.device_.productName == "NumWorks Calculator") {
                //check for ram range in descriptor of other interface
                calcModel.innerHTML = "Epsilon"
                //split interface1type with "/" and get the second part
                let ramAddress = interface1type.split("/")[1];
                if (ramAddress == "0x20000000") {
                    calcModel.innerHTML = "N0110";
                }
                if (ramAddress == "0x24000000") {
                    calcModel.innerHTML = "N0120";
                }
                
            } else if (device.device_.productName == "STM32  BOOTLOADER") {
                if (device.device_.serialNumber == "STM32FxSTM32") {
                    //try to find a better discriminator between 100/110
                    calcModel.innerHTML="RECOVERY N0100/N0110"
                }
                    
            }
                
            device.startAddress = 0x9000000C;
            const blob1 = await device.do_upload(transferSize, 16);
            //convert binary from blob to ascii
            ...
            reader.onloadend = function () {
                ...
                var version = hex.substring(0, 6);
                var patch = hex.substring(8, 15);
                //print version to version_S2
                versionS1.innerHTML = version;
                //print patch to patch_S2
                patchS1.innerHTML = patch;
            }
            device.startAddress = 0x9040000C;
            const blob = await device.do_upload(transferSize, 16);
            //convert binary from blob to ascii
            var reader = new FileReader();
            ...
            reader.onloadend = function () {
                ...
                //split hex, 6 bytes for version, space of 2 and 7 bytes for patch
                var version = hex.substring(0, 6);
                var patch = hex.substring(8, 15);
                //print version to version_S2
                versionS2.innerHTML = version;
                //print patch to patch_S2
                patchS2.innerHTML = patch;
            }
                ...
                navigator.usb.requestDevice({ 'filters': filters }).then(
                    async selectedDevice => {
                        ...
                        if (interfaces.length == 0) {
                            ...
                            //write interfaces[1] to global var
                            interface1type = interfaces[1].name;
